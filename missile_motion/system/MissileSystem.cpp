...
void MissileSystem::f(Vector &state, double time) const
{
    ...
    double height = sqrt(x*x + y*y + z*z) - Constants::Earth::MAJOR_AXIS;
    auto atm = (*atmosphere)(height);
    // TODO: write DE's

    double g = Constants::Common::G * Constants::Earth::MASS / pow(Constants::Earth::MAJOR_AXIS, 2);
    ...
    double tau = (*virtualTemperature)(height);
    ...
    double psi_d = -i_z * f_z * J_x * omega_x * state[1] / (L * m * v_k * cos(theta));
    ...
    double p_d = -g * atm.pressure * y_d / (Constants::Common::R * tau);
